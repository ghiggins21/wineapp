...
class PriceRangeWidget(RangeWidget):
    template_name = 'wineapp/price-range-slider.html'
...
class StarRatingWidget(RangeWidget):
    template_name = 'wineapp/rating-slider.html'
    ...
    def get_context(self, name, value, attrs):
        ...
        for swx, subwidget in enumerate(ctx['widget']['subwidgets']):
            subwidget['attrs']['id'] = base_id + "_" + self.suffixes[swx]
        ctx['widget']['value_text'] = "{} stars - {} stars".format(cur_min,cur_max)
        return ctx
